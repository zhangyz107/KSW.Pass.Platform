...
public abstract class StoreBase<TEntity, TKey> : IStore<TEntity, TKey>, IFilterSwitch, ITrack where TEntity : class, IKey<TKey>
{
    ...
    protected object GetKey(object id)
    {
        if (id is TKey)
            return id;
        return Helpers.Convert.To<TKey>(id);
    }
    ...
    {
        return FindByIds(Helpers.Convert.ToList<TKey>(ids));
    }
    ...
    {
        return await FindByIdsAsync(Helpers.Convert.ToList<TKey>(ids), cancellationToken);
    }
